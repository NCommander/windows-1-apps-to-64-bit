...
DWORD      time;
{
    ...
    if (GetFocus() == hWnd)
        ...
        fDoFlash = TRUE;
        SetTimer(hWnd, IDTFLASH, CMSECFLASH, (TIMERPROC)lpprocTimerFlash);
...
LRESULT CALLBACK TemplateWndProc(hWnd, message, wParam, lParam)
HWND       hWnd;
UINT   message;
WPARAM       wParam;
LPARAM       lParam;
{
    ...
    switch (message)
    {
        ...
        case WM_MOVE:
            ...
            TemplateMove(hWnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
            break;
        ...
        case WM_SIZE:
            ...
            ** SIZEFULLSCREEN when the window is being made full screen. */
            TemplateSize(hWnd, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam),
                             wParam);
            break;
