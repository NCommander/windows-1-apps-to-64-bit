/*  Type.c
    ...
    Microsoft Windows Toolkit Version 1.03
...
LRESULT CALLBACK TypeWndProc(HWND, UINT, WPARAM, LPARAM);
...
char ch;
{
    ...
    if ((ch > 31) && (ch < 127) && Curpos < 79) {
        ...
        /* Backspace */
        lOld = GetTabbedTextExtent( hDC, (LPSTR)OneLine, Curpos, 0, NULL );
        OneLine[--Curpos] = 0;
        lNew = (Curpos > 0 ? GetTabbedTextExtent(hDC, (LPSTR)OneLine, Curpos, 0, NULL) : 0L);
        rect.top    = 0;
...
HANDLE hInstance;
{
    ...
    pTypeClass->hIcon         = LoadIcon( hInstance, MAKEINTRESOURCE(TYPEICON));
...
LRESULT CALLBACK TypeWndProc( hWnd, message, wParam, lParam )
HWND hWnd;
UINT message;
WPARAM wParam;
LPARAM lParam;
{
    ...
    case WM_SYSCOMMAND:
        ...
        {
        case IDSABOUT:
            DialogBox( hInst, MAKEINTRESOURCE(ABOUTBOX), hWnd, (DLGPROC)lpprocAbout );
            break;
        default:
