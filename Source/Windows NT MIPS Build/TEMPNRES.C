...
HWND hWnd;
{
    ...
    if (fDoAsync)
        ...
        SetTimer(hWnd, IDTASYNC, CMSECASYNC, (TIMERPROC)lpprocTimerAsync);
}
/******* The code above is for demo purposes only *******/


LRESULT CALLBACK TemplateAboutDlg(hWndDlg, message, wParam, lParam)
HWND    hWndDlg;
UINT    message;
WPARAM    wParam;
LPARAM    lParam;
...
/******* The code below is for demo purposes only *******/
LRESULT WINAPI TemplateDlg(hWndDlg, message, wParam, lParam)
HWND    hWndDlg;
UINT    message;
WPARAM    wParam;
LPARAM   lParam;
...
LPPAINTSTRUCT lpps;
{
    ...
    **  aspect ratio, the lines will NOT be the same thickness */
    ...
    hpenOld = SelectObject(lpps->hdc, hpen);
    MoveToEx(lpps->hdc, rect.left  + cx, rect.top    + cy, NULL);
    LineTo(lpps->hdc, rect.right - cx, rect.top    + cy);
    MoveToEx(lpps->hdc, rect.left  + cx, rect.bottom - cy, NULL);
    ...
    MoveToEx(lpps->hdc, rect.right - cx, rect.top    + cy, NULL);
    LineTo(lpps->hdc, rect.right - cx, rect.bottom - cy);
